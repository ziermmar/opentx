...
void menuModelSetup(event_t event)
{
  ...
  MENU_TAB({
    ...
    LABEL(InternalModule),
      INTERNAL_MODULE_TYPE_ROWS,                                   // module mode (PXX(2) / None)
    ...
    switch (k) {
      ...
      case ITEM_MODEL_SETUP_INTERNAL_MODULE_TYPE: {
        ...
        if (isModuleXJT(INTERNAL_MODULE))
          ...
          lcdDrawTextAtIndex(lcdNextPos + 3, y, STR_ACCESS_RF_PROTOCOLS, g_model.moduleData[INTERNAL_MODULE].rfProtocol, menuHorizontalPosition==1 ? attr : 0);
        if (attr) {
          if (menuHorizontalPosition == 0) {
            ...
            g_model.moduleData[INTERNAL_MODULE].rfProtocol = checkIncDec(event, g_model.moduleData[INTERNAL_MODULE].rfProtocol, 0, MODULE_SUBTYPE_ISRM_PXX2_LAST, EE_MODEL, isRfProtocolAvailable);
          }
        }
#else
        uint8_t index = 0;
          ...
          if (attr && editMode > 0) {
            switch (menuHorizontalPosition) {
              case 0:
                ...
                break;
              case 1:
                CHECK_INCDEC_MODELVAR_CHECK(event, moduleData.channelsCount, -4, min<int8_t>(maxModuleChannels_M8(moduleIdx), 32-8-moduleData.channelsStart), moduleData.type == MODULE_TYPE_PXX2_ISRM ? isPXX2ChannelsCountAllowed : nullptr);
