...
enum R9MLiteFCCPowerValues {
  R9MLITE_FCC_POWER_100 = 0,
  R9MLITE_FCC_POWER_MAX = R9MLITE_FCC_POWER_100
};

enum R9MLiteLBTPowerValues {
  R9MLITE_LBT_POWER_25 = 0,
  R9MLITE_LBT_POWER_25_16,
  R9MLITE_LBT_POWER_100,
  R9MLITE_LBT_POWER_MAX = R9MLITE_LBT_POWER_100
};
...
#define BIND_TELEM_ALLOWED(idx)      (!(IS_TELEMETRY_INTERNAL_MODULE() && moduleIdx == EXTERNAL_MODULE) && (!isModuleR9M_LBT(idx) || (isModuleR9MStd(idx) && g_model.moduleData[idx].pxx.power < R9M_LBT_POWER_200) || (isModuleR9MLite(idx) && g_model.moduleData[idx].pxx.power < R9MLITE_LBT_POWER_100)))
#define BIND_CH9TO16_ALLOWED(idx)    (!isModuleR9M_LBT(idx) || (isModuleR9MStd_LBT(idx) && g_model.moduleData[idx].pxx.power != R9M_LBT_POWER_25) || (isModuleR9MLite(idx) && g_model.moduleData[idx].pxx.power != R9MLITE_LBT_POWER_25))
